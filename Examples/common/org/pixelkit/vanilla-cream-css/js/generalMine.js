...
function VanillaCreamGeneral($el) {
    var $ = jQuery;
...
// Add gradient to IE
    ...
    this.renderCheckbox = function () {
        ...
        if ($("select", $el).hasClass("select_styled")) {
            cuSel({changedEl:".select_styled", visRows:10});
        }
        if ($("div,p", $el).hasClass("input_styled")) {
            $(".input_styled input").customInput();
        }
    };
    // List Selector
    this.renderSelectList = function(){

        // First Child, Last Child
        $("li:first-child, .cusel span:first-child", $el).addClass("first");
        $("li:last-child, .cusel span:last-child", $el).addClass("last");

        // NavBar Parents Arrow
        $(".dropdown ul", $el).parent("li").addClass("parent");
        $('#listSelector li a', $el).click(function () {
            $('#listSelector li',$el).removeClass('selected');
            $('#listSelector .list-double a', $el).removeClass('border-left border-right');
            $(this).parent().addClass('selected',$el);

            if ($('#listSelector .list-multi',$el).hasClass('selected')) {
                $('#listSelector .list-double a',$el).addClass('border-right')
            }
            if ($('#listSelector .list-single',$el).hasClass('selected')) {
                $('#listSelector .list-double a',$el).addClass('border-left')
            }
        });
    }
}
